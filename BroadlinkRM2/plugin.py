...
#
"""
<plugin key="BroadlinkRM2" name="Broadlink RM2 with Kodi Remote" author="zak45" version="4.0.0" wikilink="http://www.domoticz.com/wiki/plugins/BroadlinkRM2.html" externallink="https://github.com/mjg59/python-broadlink">
    <params>
        ...
        <param field="Mode2" label="Folder to store ini files (RM2/RM mini3)" width="300px" required="true" default="/config/plugins/BroadlinkRM2"/>
...
def createIniImport():
    ...
    try:
        with open(path+"jsonSubIr", encoding='utf8') as remote_name:    
            data_remote = json.load(remote_name)
    except ValueError: # includes simplejson.decoder.JSONDecodeError
        return False

    try:
        with open(path+"jsonButton", encoding='utf8') as button_name:    
            data_button = json.load(button_name)
    except ValueError: # includes simplejson.decoder.JSONDecodeError
        return False

    try:
        with open(path+"jsonIrCode", encoding='utf8') as code_name:    
            ...
            if clear == False:
                try:
                    with open(name) as f: # No need to specify 'r': this is the default.                    
                        config = configparser.ConfigParser()
                        config.read(name,encoding='utf8')
...
# we put all switchs of the MP1 device to Off
def AllPlugOff():
...
# we put all switchs of the MP1 device to On
def AllPlugOn():
